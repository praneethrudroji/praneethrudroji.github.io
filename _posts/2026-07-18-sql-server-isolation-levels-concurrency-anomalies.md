...
A balance transfer service passed every test we threw at it. Then, in production, two concurrent transfers against the same account occasionally made money quietly disappear. Session A read a balance of 1000, added 200, and wrote back 1200. Session B, running at almost the exact same instant, also read 1000, subtracted 100, and wrote back 900. The 200 credit just vanished. The final balance should have been 1100, and it was 900. No error, no exception, no deadlock, no constraint violation. Just a quietly wrong number sitting in the table.

This is called a lost update, and it's one of four classic concurrency bugs that SQL Server's isolation levels exist to manage. It's worth learning these by name, with real scripts you can actually run, because "isolation level" isn't a setting you pick once and forget about. It's a direct trade between correctness guarantees and how many locks your transaction holds. And [lock footprint is the raw material of deadlocks](/posts/sql-server-deadlocks-deep-dive/): the isolation level you choose decides how many locks each transaction takes and for how long, which is exactly what decides how often those locks collide into a circle.
...
SERIALIZABLE is the strictest level and the most expensive one to run. It locks not just what a query actually touched, but the gaps around it too, so nothing can be inserted, updated, or deleted anywhere within that query's predicate until the transaction ends. On a range predicate over a busy table, that's a wide, long-held lock footprint. It's exactly the shape of lock that produces the [upsert-collision deadlock pattern](/posts/sql-server-deadlocks-deep-dive/#pattern-4-the-upsert-collision), where two SERIALIZABLE existence checks each grab a shared range lock and then both get stuck trying to insert into it.
...
None of these four levels, or the two versioning modes, is a universal right answer. READ UNCOMMITTED makes sense exactly when wrong data is an acceptable cost for speed. READ COMMITTED with RCSI is the right call for the overwhelming majority of OLTP (Online Transaction Processing) workloads, because it eliminates reader-writer blocking without asking a single write to change. REPEATABLE READ and SERIALIZABLE earn their keep only for the narrow set of transactions that genuinely can't tolerate a value changing underneath them mid-transaction, and they should be scoped as tightly as possible, because their cost shows up as every other session's wait time. SNAPSHOT is the right choice when a transaction needs both a consistent view of the data and the confidence that a silent lost update simply cannot happen, paid for in tempdb space and in writing retry logic for error 3960.

Knowing which anomaly each level actually closes, and being able to reproduce it yourself in two query tabs, is what turns isolation level from a setting you copy off a coworker into a decision you can actually defend.
